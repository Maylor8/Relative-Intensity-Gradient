...
##### Pre-requisites #####
  ...
  Metafiles <- "X:/Studyname/Meta files" # Where are the epoch .csv files?
...
Data <- Data %>% mutate(Bin.no = case_when(ENMO <  iOutput[1,5] ~ '2.5', 
                                           ENMO >= iOutput[1,5] & ENMO < iOutput[2,5] ~'7.5', ENMO >= iOutput[2,5] & ENMO < iOutput[3,5] ~ '12.5', 
                                           ENMO >= iOutput[3,5] & ENMO < iOutput[4,5] ~ '17.5', ENMO >= iOutput[4,5] & ENMO < iOutput[5,5] ~ '22.5', 
                                           ENMO >= iOutput[5,5] & ENMO < iOutput[6,5] ~ '27.5', ENMO >= iOutput[6,5] & ENMO < iOutput[7,5] ~ '32.5', 
                                           ENMO >= iOutput[7,5] & ENMO < iOutput[8,5] ~ '37.5', ENMO >= iOutput[8,5] & ENMO < iOutput[9,5] ~ '42.5', 
                                           ENMO >= iOutput[9,5] & ENMO < iOutput[10,5] ~ '47.5', ENMO >= iOutput[10,5] & ENMO < iOutput[11,5] ~ '52.5', 
                                           ENMO >= iOutput[11,5] & ENMO < iOutput[12,5] ~ '57.5', ENMO >= iOutput[12,5] & ENMO < iOutput[13,5] ~ '62.5', 
                                           ENMO >= iOutput[13,5] & ENMO < iOutput[14,5] ~ '67.5', ENMO >= iOutput[14,5] & ENMO < iOutput[15,5] ~ '72.5', 
                                           ENMO >= iOutput[15,5] & ENMO < iOutput[16,5] ~ '77.5', ENMO >= iOutput[16,5] & ENMO < iOutput[17,5] ~ '82.5', 
                                           ENMO >= iOutput[17,5] & ENMO < iOutput[18,5] ~ '87.5', ENMO >= iOutput[18,5] & ENMO < iOutput[19,5] ~ '92.5', 
                                           ENMO >= iOutput[19,5] & ENMO < iOutput[20,5] ~ '97.5', ENMO >= iOutput[20,5] & ENMO < iOutput[21,5] ~ '102.5', 
                                           ENMO >= iOutput[21,5] & ENMO < iOutput[22,5] ~ '107.5', ENMO >= iOutput[22,5] & ENMO < iOutput[23,5] ~ '112.5', 
                                           ENMO >= iOutput[23,5] & ENMO < iOutput[24,5] ~ '117.5', ENMO >= iOutput[24,5] & ENMO < iOutput[25,5] ~ '122.5', 
                                           ENMO >= iOutput[25,5] & ENMO < iOutput[26,5] ~ '127.5', ENMO >= iOutput[26,5] & ENMO < iOutput[27,5] ~ '132.5', 
                                           ENMO >= iOutput[27,5] & ENMO < iOutput[28,5] ~ '137.5', ENMO >= iOutput[28,5] & ENMO < iOutput[29,5] ~ '142.5', 
                                           ENMO >= iOutput[29,5] & ENMO < iOutput[30,5] ~ '147.5', ENMO >= iOutput[30,5] & ENMO < iOutput[31,5] ~ '152.5', 
                                           ENMO >= iOutput[31,5] & ENMO < iOutput[32,5] ~ '157.5', ENMO >= iOutput[32,5] & ENMO < iOutput[33,5] ~ '162.5', 
                                           ENMO >= iOutput[33,5] & ENMO < iOutput[34,5] ~ '167.5', ENMO >= iOutput[34,5] & ENMO < iOutput[35,5] ~ '172.5', 
                                           ENMO >= iOutput[35,5] & ENMO < iOutput[36,5] ~ '177.5', ENMO >= iOutput[36,5] & ENMO < iOutput[37,5] ~ '182.5', 
                                           ENMO >= iOutput[37,5] & ENMO < iOutput[38,5] ~ '187.5', ENMO >= iOutput[38,5] & ENMO < iOutput[39,5] ~ '192.5', 
                                           ENMO >= iOutput[39,5] & ENMO < iOutput[40,5] ~ '197.5', ENMO >= iOutput[40,5] & ENMO < iOutput[41,5] ~ '202.5', 
                                           ENMO >= iOutput[41,5] & ENMO < iOutput[42,5] ~ '207.5', ENMO >= iOutput[42,5] & ENMO < iOutput[43,5] ~ '212.5', 
                                           ENMO >= iOutput[43,5] & ENMO < iOutput[44,5] ~ '217.5', ENMO >= iOutput[44,5] & ENMO < iOutput[45,5] ~ '222.5', 
                                           ENMO >= iOutput[45,5] & ENMO < iOutput[46,5] ~ '227.5', ENMO >= iOutput[46,5] & ENMO < iOutput[47,5] ~ '232.5', 
                                           ENMO >= iOutput[47,5] & ENMO < iOutput[48,5] ~ '237.5', ENMO >= iOutput[48,5] & ENMO < iOutput[49,5] ~ '242.5', 
                                           ENMO >= iOutput[49,5] & ENMO < iOutput[50,5] ~ '247.5', ENMO >= iOutput[50,5] & ENMO < iOutput[51,5] ~ '252.5', 
                                           ENMO >= iOutput[51,5] & ENMO < iOutput[52,5] ~ '257.5', ENMO >= iOutput[52,5] & ENMO < iOutput[53,5] ~ '262.5', 
                                           ENMO >= iOutput[53,5] & ENMO < iOutput[54,5] ~ '267.5', ENMO >= iOutput[54,5] & ENMO < iOutput[55,5] ~ '272.5', 
                                           ENMO >= iOutput[55,5] & ENMO < iOutput[56,5] ~ '277.5', ENMO >= iOutput[56,5] & ENMO < iOutput[57,5] ~ '282.5', 
                                           ENMO >= iOutput[57,5] & ENMO < iOutput[58,5] ~ '287.5', ENMO >= iOutput[58,5] & ENMO < iOutput[59,5] ~ '292.5', 
...
write.csv(x = MasterOut, file = "relative intensity daysummary.csv",row.names = F)  
